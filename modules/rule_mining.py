...
def mine_rules_from_file(path: Path, max_items: int = 120):
    t = docx_text(path) if path.suffix.lower()==".docx" else pdf_text(path) or ""
    cands = [s for s in sentences(t) if HINT.search(s)]
    rows = []
    for s in cands[:max_items]:
        desc = s[:200]
        toks = re.findall(r'[A-Za-z0-9\-]{3,}', s)[:10]
        pattern = '\\b' + '\\s+'.join([re.escape(t) for t in toks[:5]]) + '\\b' if toks else re.escape(desc[:40])
        rows.append({
