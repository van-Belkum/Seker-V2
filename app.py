...
def page_audit():
    ...
    if st.button("Run Audit", type="primary", use_container_width=True, disabled=(up is None)):
        ...
        pages_text = rules["__extracted_text"]  # rules_engine extracts+attaches text from PDF
        findings = run_rule_checks(pages_text, meta, rules)

        # status
        status = "PASS" if len([f for f in findings if f.get("severity","minor")=="major"])==0 and len(findings)==0 else "REJECTED"
...
def page_training():
    ...
    if up:
        df = pd.read_excel(up)
        # Expect columns: title, reason, page, bbox(optional), verdict, rule_update(optional), plus metadata sheet
...
def page_analytics():
    ...
    cutoff = (dt.datetime.utcnow() - dt.timedelta(days=14)).isoformat(timespec="seconds")
    if "timestamp_utc" in df:
        df = df[pd.to_datetime(df["timestamp_utc"], errors="coerce") >= pd.to_datetime(cutoff)]
    if "exclude" in df:
    ...
    st.dataframe(df[["timestamp_utc","supplier","client","project","status","pdf_name","excel_name","n_findings"]].sort_values("timestamp_utc", ascending=False), use_container_width=True)
...
def page_admin():
    ...
    with tab1:
        ...
        tags = st.text_input("Tags (comma)", value="").split(",") if st.text_input else []
