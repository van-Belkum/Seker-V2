...
st.set_page_config(page_title="Seker V3 — Restored + AI Guidance", layout="wide", page_icon="🛰️")
...
with tabs[0]:
    ...
    # Design audit
    with sub[0]:
        st.subheader("Design Audit — V1 fields restored")
        ui = settings.get("ui",{})
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            supplier = st.selectbox("Supplier", ui.get("suppliers",[]), index=0 if ui.get("suppliers") else None)
            client   = st.selectbox("Client", ui.get("clients",[]), index=0 if ui.get("clients") else None)
        with col2:
            project  = st.selectbox("Project", ui.get("projects",[]), index=0 if ui.get("projects") else None)
            site_type= st.selectbox("Site Type", ui.get("site_types",[]), index=0 if ui.get("site_types") else None)
        with col3:
            vendor   = st.selectbox("Vendor", ui.get("vendors",[]), index=0 if ui.get("vendors") else None)
            cab_loc  = st.selectbox("Cabinet Location", ui.get("cabinet_locations",[]), index=0 if ui.get("cabinet_locations") else None)
        with col4:
            radio_loc= st.selectbox("Radio Location", ui.get("radio_locations",[]), index=0 if ui.get("radio_locations") else None)
        ...
        if show_mimo:
            ...
            s1 = st.selectbox("S1 MIMO", mimo_opts, index=1 if len(mimo_opts)>1 else 0)
            mimo_vals["mimo_S1"] = s1
            for s in ["S2","S3","S4"]:
                mimo_vals[f"mimo_{s}"] = s1 if use_all else st.selectbox(f"{s} MIMO", mimo_opts, index=1 if len(mimo_opts)>1 else 0)
        else:
            st.info(f"MIMO selections hidden for project '{hide_mimo_for}'.")

        st.divider()
        site_address = st.text_input("Site Address * (', 0 ,' will be ignored in title check)", "")
        drawing_title= st.text_input("Drawing Title *", "")
        checklist_up = st.file_uploader("Checklist (Excel optional)", type=["xlsx","xls","csv"])
        design_pdf   = st.file_uploader("Design PDF (for rule markers & annotation)", type=["pdf"])
        exclude_analytics = st.checkbox("Exclude this run from analytics", value=False)
        submitted = st.button("Run Design Audit")

        if submitted:
            if not site_address or not drawing_title:
                st.error("Please complete required fields: Site Address and Drawing Title.")
            else:
                # load checklist minimal
                if checklist_up is None:
                    st.warning("Using templates/checklist_template.xlsx")
                    import pandas as _pd
                    checklist_df = _pd.read_excel("templates/checklist_template.xlsx")
                else:
                    if checklist_up.name.lower().endswith(".csv"):
                        import pandas as _pd
                        checklist_df = _pd.read_csv(checklist_up)
                    else:
                        import pandas as _pd
                        checklist_df = _pd.read_excel(checklist_up)

                # address-title check ignoring ', 0 ,'
                def clean(s): return (s or "").replace(", 0 ,", ",").strip().lower()
                errs = []
                ...
                if design_pdf is not None:
                    ...
                    if matches:
                        ...
                        from modules.pdf_annotate import annotate_text_matches
                        annotate_text_matches(p, outp, matches); annotated_paths.append(outp)
                ...
                for ap in annotated_paths:
                    with open(ap,"rb") as f:
                        st.download_button("Download Auto-Annotated PDF", data=f, file_name=ap.name, mime="application/pdf")

                # history
                payload = {
                    "When": pd.Timestamp.now().isoformat(timespec="seconds"),
                    "Project": project, "Client": client, "Supplier": supplier, "Vendor": vendor,
                    "Site Address": site_address, "Drawing Title": drawing_title,
                    "Errors": len(errs), "Excluded": bool(exclude_analytics),
        ...
        if pdf2:
            ...
            import fitz
            d = fitz.open(str(p2)); n = len(d); d.close()
    ...
    # Document audit
    with sub[1]:
        st.subheader("Audit DOCX/PDF against rules")
        ...
        with col1:
            ...
            if (not privacy_hide) or is_admin(token):
                if index_file.exists():
                    ...
                    st.info("No index yet. Go to Train to upload & index guidance.")
            else:
...
# ---------------- TRAIN (ADMIN) ----------------
with tabs[1]:
    st.header("Train (Admin) — Guidance & AI Rule Miner")
    if not is_admin(token):
        ...
        with colx: overwrite = st.checkbox("Overwrite same filename", value=False)
        with coly: supersede = st.checkbox("Supersede older versions by key", value=True)
        with colz: mode = st.selectbox("Index mode", ["append","overwrite"], index=0)
        if st.button("Save & Index"):
            ...
            for f in files or []:
                ...
                if p.exists() and not overwrite:
                    p = g_root / f"__{int(time.time())}__{f.name}"
                p.write_bytes(f.read()); saved += 1
            out = index_folder(g_root, index_file, mode=mode, supersede=supersede)
            st.success(f"Saved {saved} file(s). Indexed → {out}")

        st.divider()
        st.subheader("AI-like Rule Miner from Guidance")
        if index_file.exists():
            idx = pd.read_csv(index_file)
            pick = st.selectbox("Pick a guidance file to mine", [""] + idx["file"].tolist())
            if pick:
                path = g_root / pick
                ...
                if mined.empty:
                    ...
                    if st.button("Append top 20 mined rules to ruleset"):
                        rs = load_ruleset()
                        ...
                        st.success("Appended mined rules. Review/edit in Settings.")
...
with tabs[2]:
    ...
    if dfh.empty:
        ...
        if "Errors" in dfh.columns:
            st.subheader("Trend — Errors per run")
            fig, ax = plt.subplots()
            dfh["Errors"].plot(kind="line", ax=ax)
            ax.set_xlabel("Run index"); ax.set_ylabel("Errors")
            st.pyplot(fig)
